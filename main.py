...
class Player():
    def __init__(self, x, y, width, height, change_x, change_y, direction):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
    ...
    def draw_player_up(self, screen):
    ...
    def draw_player_down(self, screen):
        pygame.draw.ellipse(screen, WHITE, [self.x, self.y, self.width, self.height])
        pygame.draw.rect(screen, RED, [self.x + (self.width / 2.25), self.y + (self.height / 2), self.width / 8, self.height / 2])
    def draw_player_left(self, screen):
        pygame.draw.ellipse(screen, WHITE, [self.x, self.y, self.width, self.height])
        pygame.draw.rect(screen, RED, [self.x, self.y + (self.height / 2.25), self.width / 2, self.height / 8])
    def draw_player_right(self, screen):
        pygame.draw.ellipse(screen, WHITE, [self.x, self.y, self.width, self.height])
        pygame.draw.rect(screen, RED, [self.x + (self.width / 2), self.y + (self.height / 2.25), self.width / 2, self.height / 8])
def main():
    # Initialize pygame
    pygame.init()
    ...
    bullets = []
        ...
        # Player
    player = Player(SCREEN_WIDTH / 2, SCREEN_HEIGHT - 100, 50, 50, 0, 0, "up")
        # Screen
    ...
    while not done:
        for event in pygame.event.get():
            ...
            # when player clicks create a circle in the bullets list
                ...
                if player.direction == "up":
                    ...
                    bullets.append(Circle(player.x + (player.width / 2.5), player.y + player.height, 10, 10, WHITE, 0, 6, 0))
                elif player.direction == "left":
                    bullets.append(Circle(player.x, player.y + (player.height / 2.5), 10, 10, WHITE, -6, 0, 0))
                elif player.direction == "right":
                    bullets.append(Circle(player.x + player.width, player.y + (player.height / 2.5), 10, 10, WHITE, 6, 0, 0))
        ...
        # draw player
        if player.direction == "up":
            player.draw_player_up(screen)
        elif player.direction == "down":
            player.draw_player_down(screen)
        elif player.direction == "left":
            player.draw_player_left(screen)
        elif player.direction == "right":
            player.draw_player_right(screen)
        # Draw Circles
        for i in range(len(random_circles)):
